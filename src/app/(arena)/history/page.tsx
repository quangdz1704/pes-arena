...
export default async function HistoryPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-7"><PageHeading eyebrow="Thống kê" title="Lịch sử" description="Các trận đã hoàn tất, lưu trực tiếp từ sân PES vào PostgreSQL." action={<Button asChild className="rounded-xl font-bold"><Link href="/matches/new"><Plus className="size-4" /> Trận mới</Link></Button>} />{!databaseReady ? <DatabaseSetupNotice /> : history.length === 0 ? <Card className="border-dashed bg-transparent"><CardContent className="flex flex-col items-center py-16 text-center"><History className="mb-4 size-10 text-muted-foreground" /><h2 className="font-bold">Chưa có trận nào được lưu.</h2><p className="mt-1 text-sm text-muted-foreground">Đá trận đầu tiên để bắt đầu lịch sử đau thương.</p><Button asChild className="mt-5"><Link href="/matches/new">Tạo trận đầu tiên</Link></Button></CardContent></Card> : <div className="space-y-3">{history.map((match) => <Link key={match.id} href={`/matches/${match.id}`} className="block"><Card className="border-white/10 bg-card/80 transition hover:border-primary/40 hover:bg-card"><CardContent className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 py-5 text-center sm:gap-3"><div className="min-w-0"><p className="truncate font-bold">{match.sides[0].players.map((player) => player.name).join(" + ")}</p><p className="mt-1 truncate text-xs text-muted-foreground">{match.sides[0].team?.name}</p></div><div><p className="whitespace-nowrap text-2xl font-black tabular-nums sm:text-3xl">{match.sides[0].score} <span className="text-muted-foreground">-</span> {match.sides[1].score}</p><div className="mt-2 flex justify-center gap-1"><Badge variant="outline">{match.matchMode === "ONE_V_ONE" ? "1v1" : "2v2"}</Badge>{!match.isRanked ? <Badge className="hidden sm:inline-flex" variant="secondary">Không xếp hạng</Badge> : null}</div></div><div className="min-w-0"><p className="truncate font-bold">{match.sides[1].players.map((player) => player.name).join(" + ")}</p><p className="mt-1 truncate text-xs text-muted-foreground">{match.sides[1].team?.name}</p></div><p className="col-span-3 text-xs text-muted-foreground">{formatPlayedAt(match.playedAt)}</p></CardContent></Card></Link>)}</div>}</div>;
}
