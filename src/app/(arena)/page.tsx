                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">
                        {activeMatch.sides.some((side) => side.score !== null)
                          ? "Tỉ số live"
                          : "Chưa chốt tỉ số"}
                      </span>
                    </div>
                    <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-2 text-center">
                      {activeMatch.sides.map((side, index) => (
                        <div
                          key={side.id}
                          className={index === 1 ? "contents" : undefined}
                        >
                          {index === 1 ? (
                            <p className="text-lg font-black text-primary">
                              {activeMatch.sides[0].score ?? 0} -{" "}
                              {activeMatch.sides[1].score ?? 0}
                            </p>
                          ) : null}
                    ...
                    <Button
                      asChild
                      className="mt-5 h-10 w-full rounded-xl font-black"
                    >
                      <Link href={`/matches/${activeMatch.id}`}>
                        <Play className="size-4" /> Tiếp tục & nhập tỉ số
                      </Link>
                    </Button>
                  </CardContent>
                </Card>
        ...
        <Card className="border-dashed border-white/12 bg-transparent">
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Bắt đầu một kèo mới, sau đó bạn luôn có thể quay lại đây để nhập
              tỉ số.
            </p>
          </CardContent>
        </Card>
